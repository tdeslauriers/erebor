...
# variables
NAMESPACE="world"
SECRET_NAME="secret-gateway-db-client-cert"

# get certificate and key from 1Password
CLIENT_CERT=$(op document get "erebor_db_client_prod_cert" --vault world_site | base64 -w 0)
CLIENT_KEY=$(op document get "erebor_db_client_prod_key" --vault world_site | base64 -w 0)

# check if values are retrieved successfully
if [[ -z "$CLIENT_CERT" || -z "$CLIENT_KEY" ]]; then
  echo "Error: failed to get gateway prod db client certificate or key from 1Password."
  exit 1
fi
